...
/**
 * 基于红黑树的NavigableMap实现。该Map根据key的自然排序顺序进行排序，或根据创建Map时提供的Comparator进行排序，具体取决于使用的构造函数。
 * 此实现保证containsKey、get、put和remove操作的时间复杂度为log(n)。算法改编自Cormen、Leiserson和Rivest的《算法导论》。
 *
 * 需注意：TreeMap（与任何SortedMap类似）所维护的排序关系，无论是否提供显式比较器，都必须与equals方法保持一致，才能正确实现Map接口。
 * （关于“与equals一致”的精确定义，请参阅Comparable或Comparator说明。） 这是因为Map接口基于equals操作定义，而排序映射通过compareTo（或compare）方法执行所有键值比较——该方法判定相等的两个键，在排序映射中即视为等价。即使排序映射的排序规则与equals不一致，其行为仍属规范范畴，只是未能遵守Map接口的通用契约。
 *
 * 本类及其视图方法返回的所有Map.Entry对都代表生成时的映射快照。它们不支持Entry.setValue方法。（但请注意，可通过put方法修改关联映射中的映射关系。）
 * @author liyibo
 * @date 2026-02-25 17:16
 */
public class TreeMap<K, V> extends AbstractMap<K, V> implements NavigableMap<K, V>, Cloneable, Serializable {
    ...
    private V put(K key, V value, boolean replaceOld) {
        ...
        // 到这里说明t == null了，parent指向最后一级节点，直接新增Entry
        addEntry(key, value, parent, cmp < 0);
        return null;
    }
