...
/**
 * 本类仅包含操作集合或返回集合的静态方法，包含了作用于集合的多态算法、包装器（返回由指定集合支撑的新集合）以及若干其他零碎功能。
 * 若传入的集合或类对象为空，本类所有方法均会抛出NullPointerException。
 * 本类中多态算法的文档通常包含简要的实现说明。此类说明应视为实现注释，而非规范组成部分。
 * 实现者可自由替换其他算法，只要符合规范要求即可（例如排序算法不必采用归并排序，但必须保持稳定性）。
 *
 * 本类中包含的“破坏性”算法（即会修改操作对象集合的算法）规定：若集合不支持相应的修改操作（如set方法），则抛出UnsupportedOperationException。
 * 当调用操作对集合无效时，这些算法可选择性抛出该异常，但并非强制要求。
 * 例如，对已排序的不可修改列表调用排序方法时，可能抛出也可能不抛出UnsupportedOperationException。
 * @author liyibo
 * @date 2026-03-03 14:27
 */
public class Collections {
    ...
    /** 二分查找算法的容量阈值（元素数量少于5000调用indexedBinarySearch，大于则调用iteratorBinarySearch） */
    private static final int BINARYSEARCH_THRESHOLD = 5000;

    /** 反转List的算法阈值，低于阈值则直接使用swap方法来反转 */
    private static final int REVERSE_THRESHOLD = 18;

    /** 乱序List的算法阈值，低于阈值则直接使用swap方法来乱序 */
    ...
    /** indexOfSubList方法的算法阈值 */
    private static final int INDEXOFSUBLIST_THRESHOLD = 35;


    /**
     * 本质上是用List的default方法进行的排序
     */
    ...
    /**
     * 将指定list的所有元素替换为指定元素，该方法运行时间为线性。
     */
    public static <T> void fill(List<? super T> list, T obj) {
        int size = list.size();
        if(size < FILL_THRESHOLD || list instanceof RandomAccess) {
            for(int i = 0; i < size; i++)   // 直接遍历赋值
                list.set(i, obj);
        }else {
    ...
    public static <T> T min(Collection<? extends T> col, Comparator<? super T> comparator) {
        if(comparator==null)
            return (T)min((Collection) col);
        Iterator<? extends T> iter = col.iterator();
        T candidate = iter.next();
        while(iter.hasNext()) {
            T next = iter.next();
    ...
    public static <T> T max(Collection<? extends T> col, Comparator<? super T> comparator) {
        if (comparator==null)
            return (T)max((Collection)col);
        Iterator<? extends T> iter = col.iterator();
        T candidate = iter.next();
        while(iter.hasNext()) {
            T next = iter.next();
    ...
    /**
     * 返回一个不可变的List，其中包含指定对象的n个副本。
     * 该方法与List.addAll方法配合使用时，可以用于扩展List。
     */
    ...
    private static class CopiesList<E> extends java.util.AbstractList<E> implements RandomAccess, Serializable {
        @java.io.Serial
    ...
    public static <T> Enumeration<T> enumeration(final Collection<T> c) {
        return new Enumeration<T>() {
            private final Iterator<T> iter = c.iterator();
